...
- [2026-07-29 - Hero arcs v2: the budgets braid around the plate line](2026-07-29-hero-braid.md) - variant B chosen from a two-cut preview; wine spine keeps its choreography, baked braid polylines, whole-svg lightning field gated to desktop + motion-allowed, ghost arc deleted.
- [2026-07-29 - Secondary pages onto the landing brand, and the half-fix line](2026-07-29-secondary-page-brand-pass.md) - the three tiers of brand staleness a sweep must expect (never swept / half swept / token-lagged), why the public /design/ brand kit was deliberately NOT half-fixed, and the committed debug toggle that had put REVIEW badges on the public legal page.
- [2026-07-29 - HTML entry-point caching: the max-age=300 "fix" was the bug](2026-07-29-html-cache-freshness.md) - the pages `_headers` covered were the only stale ones (the platform default is already max-age=0), why `?v=N` cannot version an entry URL, five options deliberated, and the rollout-propagation caveat no header can fix.
